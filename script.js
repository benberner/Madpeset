...
// === v6.3 texts.json loader (production) ===
async function loadTexts(){
  try{
    const res = await fetch('data/texts.json?v=6.3', { cache:'no-store' });
    if (!res.ok) return;
    ...
    if (t.step2){
      ...
      if (root){
        ...
        if (t.step2.opt_black_unavailable) setAfterInputLabel(root, 'input[value="black_unavailable"]', t.step2.opt_black_unavailable);
        if (t.step2.opt_macaron_soon) setAfterInputLabel(root, 'input[value="macaron_soon"]', t.step2.opt_macaron_soon);
...
// Ensure run
document.addEventListener('DOMContentLoaded', ()=>{ try{ loadTexts(); }catch(e){} });
